...
RSpec.describe DataPorter::ImportJob do
  let(:target_class) do
  ...
  it "calls Orchestrator#import!" do
    orchestrator = instance_double(DataPorter::Orchestrator, import!: nil)
    allow(DataPorter::Orchestrator).to receive(:new).and_return(orchestrator)

    described_class.new.perform(data_import.id)

    expect(orchestrator).to have_received(:import!)
  end
